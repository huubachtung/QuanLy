...
class LeaveRequestPage extends StatefulWidget {
  ...
  @override State<LeaveRequestPage> createState() => _LeaveRequestPageState();
}
...
class _LeaveRequestPageState extends State<LeaveRequestPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...user.leaveBalances.where((lb) => lb.leaveType != 'ANNUAL_LEAVE').map((lb) =>
          _LeaveBalanceRow(balance: lb, isDark: isDark)),
        ...
        BlocBuilder<LeaveBloc, LeaveState>(
          builder: (context, state) {
            ...
            if (state is LeaveLoaded) {
              ...
              if (recent.isEmpty) return const EmptyState(icon: Icons.inbox_rounded, title: 'Chưa có đơn nào');
              return Column(children: recent.map((r) => _RequestCard(request: r, isDark: isDark)).toList());
            }
  ...
  Widget _buildAnnualLeaveCard(BuildContext context, UserModel user, bool isDark) {
    ...
    Color badgeColor;
    String badgeLabel;
    if (remaining <= 0) { badgeColor = AppColors.error; badgeLabel = 'Hết phép'; }
    else if (remaining <= 2 || remaining <= annualBalance.totalDays * 0.2) {
      badgeColor = AppColors.warning; badgeLabel = 'Sắp hết';
    } else { badgeColor = AppColors.success; badgeLabel = 'Còn nhiều'; }

    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [AppColors.primaryBlue, Color(0xFF1E4A9A)],
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Text('Phép năm', style: TextStyle(color: Colors.white70, fontSize: 13)),
          const Spacer(),
          StatusBadge(label: badgeLabel, color: badgeColor, textColor: badgeColor),
          if (annualBalance.pendingDays > 0) ...[
            const SizedBox(width: 8),
            StatusBadge(label: '+${annualBalance.pendingDays} chờ duyệt', color: AppColors.gold),
          ],
        ]),
        const SizedBox(height: 12),
        Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          const Padding(padding: EdgeInsets.only(bottom: 8),
            child: Text('/ 12 ngày', style: TextStyle(color: Colors.white70, fontSize: 13))),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          _LeaveStatChip('Đã dùng', '${annualBalance.usedDays}', Colors.white.withValues(alpha: 0.25)),
          const SizedBox(width: 8),
          _LeaveStatChip('Còn lại', '$remaining', Colors.white.withValues(alpha: 0.25)),
          const SizedBox(width: 8),
          if (annualBalance.pendingDays > 0)
            _LeaveStatChip('Chờ duyệt', '${annualBalance.pendingDays}', AppColors.gold.withValues(alpha: 0.3)),
        ]),
      ]),
    );
  }
...
class _LeaveBalanceRow extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) {
    return Container(
      ...
      child: Row(children: [
        Expanded(child: Text(balance.label,
          style: Theme.of(ctx).textTheme.bodyMedium, overflow: TextOverflow.ellipsis)),
        const SizedBox(width: 8),
        ...
        Text('Còn ${balance.remainingDays.toInt()}',
          style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
            color: balance.remainingDays <= 0 ? AppColors.error : AppColors.success)),
      ]),
    );
  }
}
    ...
    return Container(
      ...
      child: Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(request.leaveType.label, style: Theme.of(ctx).textTheme.titleSmall, maxLines: 1, overflow: TextOverflow.ellipsis),
...
class _CreateLeaveSheetState extends State<_CreateLeaveSheet> {
  ...
  List<LeaveType> get _leaveTypes => _group == 'leave'
    ? [LeaveType.annualLeave, LeaveType.previousYearLeave, LeaveType.compensatoryLeave,
       LeaveType.sickLeave, LeaveType.summerLeave, LeaveType.unpaidLeave,
       LeaveType.marriageLeave, LeaveType.bereavementLeave, LeaveType.wifeBirthSingleNormal]
        ...
        return DraggableScrollableSheet(
          ...
          builder: (_, ctrl) => Container(
            ...
            child: ListView(controller: ctrl, padding: const EdgeInsets.all(24), children: [
              ...
              Text('Tạo đơn mới', style: Theme.of(context).textTheme.headlineMedium),
              const SizedBox(height: 20),
              // Group selector
              Row(children: [
              ...
              Wrap(spacing: 8, runSpacing: 8, children: _leaveTypes.map((t) => GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: _selectedType == t ? AppColors.primaryBlue : Colors.transparent,
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(color: _selectedType == t
                      ? AppColors.primaryBlue : (widget.isDark ? AppColors.darkBorder : AppColors.lightBorder)),
                  ),
                  child: Text(t.label,
                    style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500,
                      color: _selectedType == t ? Colors.white : null)),
                ))).toList()),
              const SizedBox(height: 16),
              // Date / Time fields
              if (!_isTimeInput) ...[
                ...
                if (!_selectedType.isSpecialRequest || _selectedType == LeaveType.onlineWork) ...[
                  ...
                  Row(children: LeaveDuration.values.map((d) => Expanded(child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: _duration == d ? AppColors.primaryBlue : Colors.transparent,
                        borderRadius: BorderRadius.circular(10),
                        border: Border.all(color: _duration == d
                          ? AppColors.primaryBlue : (widget.isDark ? AppColors.darkBorder : AppColors.lightBorder)),
                      ),
                      child: Text(d.label, textAlign: TextAlign.center,
                        style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                          color: _duration == d ? Colors.white : null)),
                    )))).toList()),
                  const SizedBox(height: 12),
                ],
              ] else ...[
              ...
              SizedBox(width: double.infinity, child: ElevatedButton(
                ...
                child: submitting
                  ...
                  : const Text('Gửi đơn', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
              )),
            ]),
          ),
        );
  ...
  @override Widget build(BuildContext ctx) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: selected ? AppColors.primaryBlue : AppColors.darkBorder),
      ),
      child: Center(child: Text(groupLabel, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
        color: selected ? Colors.white : null), textAlign: TextAlign.center)),
    ),
  );
}
  ...
  @override Widget build(BuildContext ctx) => GestureDetector(
    onTap: () async {
      final d = await showDatePicker(context: ctx,
        initialDate: value, firstDate: DateTime(2024), lastDate: DateTime(2028));
