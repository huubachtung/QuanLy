...
class AssetDetailPage extends StatelessWidget {
          ...
          if (state is AssetLoaded) {
            ...
            return SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text(asset.assetCode, style: Theme.of(context).textTheme.bodySmall),
                      ])),
                ...
                // Details
                _Section('Thông tin thiết bị', [
                  _Row('Danh mục', asset.category),
                  _Row('Thương hiệu', asset.brand),
                  _Row('Model', asset.model),
                  if (asset.serialNumber.isNotEmpty) _Row('Serial', asset.serialNumber),
                  _Row('Vị trí', asset.location.isEmpty ? 'Chưa cập nhật' : asset.location),
                ], isDark, context),
                const SizedBox(height: 12),
                _Section('Thông tin mua sắm', [
                  if (asset.purchaseDate != null) _Row('Ngày mua', fmt.format(asset.purchaseDate!)),
                  if (asset.purchasePrice > 0)
                    _Row('Giá mua', NumberFormat('#,###', 'vi').format(asset.purchasePrice) + ' VNĐ'),
                  if (asset.warrantyExpiry != null)
                    _Row('Bảo hành đến', fmt.format(asset.warrantyExpiry!),
                      valueColor: asset.warrantyExpiry!.isBefore(DateTime.now()) ? AppColors.error : null),
                ], isDark, context),
                const SizedBox(height: 12),
                _Section('Lịch sử bàn giao', [
                  _Row('Ngày nhận', asset.assignedAt != null ? fmt.format(asset.assignedAt!) : 'N/A'),
                ], isDark, context),
  ...
  Widget _Section(String title, List<Widget> rows, bool isDark, BuildContext ctx) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: isDark ? AppColors.darkCard : AppColors.lightCard,
...
class _Row extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) => Padding(
    ...
    child: Row(children: [
      SizedBox(width: 120, child: Text('$label:', style: Theme.of(ctx).textTheme.bodySmall)),
